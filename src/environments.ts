  ...
  for (const provider of PROVIDERS) {
    ...
    if (providerConfig.instructions !== undefined) {
      ...
      for (const sourceName of names) {
        sources.push(await resolveSource(manifestDirectory, sourceName));
      }
      const composition = compose({
        snippets: await Promise.all(
          sources.map(async (source) => ({
            source,
            content: normalizeLegacySnippetCommandDuplicates(await readFile(source, "utf8")),
          })),
        ),
...
function normalizeLegacySnippetCommandDuplicates(content: string): string {
  const opening = /^\+\+\+\r?\n/.exec(content);
  ...
  try {
    metadata = { ...parseToml(rest.slice(0, closing.index)) };
  } catch {
    return content;
  }

  let changed = false;
  for (const field of ["requires", "excludes"] as const) {
    const value = metadata[field];
    if (!Array.isArray(value) || !value.every((entry) => typeof entry === "string")) continue;
    const commands = value as string[];
    const unique = [...new Set(commands)];
    if (unique.length === commands.length) continue;
    metadata[field] = unique;
    changed = true;
  }
  if (!changed) return content;

  const lineEnding = opening[0].endsWith("\r\n") ? "\r\n" : "\n";
  const frontMatter = stringifyToml(metadata).replaceAll("\n", lineEnding);
  const body = rest.slice(closing.index + closing[0].length);
  return `${opening[0]}${frontMatter}+++${lineEnding}${body}`;
}
