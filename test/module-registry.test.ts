...
test("removal requires force while a runtime composition exists", async () => {
  const fixture = await registryFixture();
  const requiresForce =
    /Cannot prove module alpha is unused while jaeger\.runtime\.mjs exists/;
  try {
    await addModules({
      root: fixture.runtimeRoot,
      references: [`${fixture.catalogPath}#alpha`],
    ...
    await writeFile(
      ...
      'const dir = "./" + "modules"\n' +
        'const name = "alpha"\n' +
        'const alpha = await import(`${dir}/${name}/alpha.mjs`)\n' +
    ...
    await writeFile(
      ...
      'const root = process.cwd()\n' +
        'const alpha = await import(pathToFileURL(path.join(root, "modules", "alpha", "alpha.mjs")).href)\n' +
        "export default { version: 1, modules: [alpha] }\n",
    );
    await assert.rejects(
      removeModules({
    ...
    await writeFile(
      ...
      'const modulesDir = path.join(import.meta.url, "modules")\n' +
        'const alphaPath = path.join(modulesDir, "alpha", "alpha.mjs")\n' +
        "const alpha = await import(alphaPath)\n" +
        "export default { version: 1, modules: [alpha] }\n",
    );
    await assert.rejects(
      removeModules({
