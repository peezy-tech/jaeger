...
test("environment defaults use the canonical mdcsp configuration home", () => {
  assert.deepEqual(
    defaultEnvironmentPaths({
    ...
    {
      configRoot: path.resolve("/config/mdcsp"),
      stateRoot: "/state/jaeger/environment",
      jaegerConfigRoot: "/config/jaeger",
    },
  );
  assert.equal(
    defaultEnvironmentPaths({ MDCSP_HOME: "/custom/mdcsp" }).configRoot,
    path.resolve("/custom/mdcsp"),
  );
});

test("default environments compose the same canonical mdcsp profile and snippets", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "jaeger-mdcsp-sidecar-"));
  const configRoot = path.join(root, "config", "mdcsp");
  const stateRoot = path.join(root, "state", "jaeger", "environment");
  const codexHome = path.join(root, "codex");
  const claudeHome = path.join(root, "claude");
  const profilePath = path.join(configRoot, "profiles", "workstation.toml");
  const claudeProfilePath = path.join(configRoot, "profiles", "claude.toml");
  const snippetPath = path.join(configRoot, "snippets", "common.md");
  const claudeSnippetPath = path.join(configRoot, "snippets", "claude.md");
  const sidecarRoot = path.join(configRoot, "jaeger", "workstation");
  try {
    await mkdir(path.dirname(profilePath), { recursive: true });
    await mkdir(path.dirname(snippetPath), { recursive: true });
    await mkdir(path.join(sidecarRoot, "skills", "example-skill"), { recursive: true });
    await writeFile(
      profilePath,
      `version = 1\nname = "workstation"\nsnippets = ["common"]\n`,
    );
    ...
    await writeFile(snippetPath, "# Shared instructions\n");
    await writeFile(claudeSnippetPath, "# Claude instructions\n");
    await writeFile(
      path.join(sidecarRoot, "environment.toml"),
      `version = 1\nname = "workstation"\n` +
        `[providers.codex]\nskills = ["skills/example-skill"]\n` +
        `[providers.claude]\nprofile = "claude"\n`,
    );
    await writeFile(
      path.join(sidecarRoot, "skills", "example-skill", "SKILL.md"),
      "# Example skill\n",
    );

    const plan = await loadEnvironmentPlan(
      "workstation",
      { configRoot, stateRoot, jaegerConfigRoot: path.join(root, "config", "jaeger") },
      undefined,
      { CODEX_HOME: codexHome, CLAUDE_CONFIG_DIR: claudeHome, PATH: "" },
    );
    const instructions = plan.resources.find((resource) => resource.category === "instructions");
    assert.equal(plan.manifestPath, profilePath);
    assert.equal(instructions?.target, path.join(codexHome, "AGENTS.md"));
    assert.equal(
      instructions?.content,
      `<!-- Generated by mdcsp. Edit snippets, not this file.\n` +
        `     Profile: workstation (${profilePath})\n` +
        `     Content: sha256:afcf8d9ed830 -->\n\n` +
        `# Shared instructions\n`,
    );
    assert.equal(
    ...
    assert.match(
      plan.resources.find(
        (resource) => resource.provider === "claude" && resource.category === "instructions",
      )?.content ?? "",
      /# Claude instructions/,
    );
    assert.deepEqual(plan.snippets, [
      { path: snippetPath, selected: true, reason: "selected" },
      { path: claudeSnippetPath, selected: true, reason: "selected" },
    ]);
  } finally {
...
test("Jaeger sidecars cannot replace the environment's canonical Codex profile", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "jaeger-mdcsp-authority-"));
  const configRoot = path.join(root, "mdcsp");
  const profilePath = path.join(configRoot, "profiles", "default.toml");
  const sidecarPath = path.join(configRoot, "jaeger", "default", "environment.toml");
  try {
    await mkdir(path.dirname(profilePath), { recursive: true });
    ...
    await assert.rejects(
      () => loadEnvironmentPlan(
        "default",
        { configRoot, stateRoot: path.join(root, "state") },
      ),
      /mdcsp profile/,
    );
  } finally {
...
test("environment plans preserve legacy bytes when no instruction body is selected", async () => {
  const cases = [
    {
    ...
    {
      name: "excluded_instructions",
      instructions: '["snippets/conditional.md"]',
      files: {
        "snippets/conditional.md": `+++
requires = ["definitely-not-installed"]
+++
...
`,
  ...
  for (const fixture of cases) {
    await withEnvironment(async ({ root, paths, codexHome }) => {
      const directory = await writeEnvironment(root, fixture.name, {
        manifest: `
version = 1
name = "${fixture.name}"
...
`,
      ...
      const manifestPath = path.join(directory, "environment.toml");
      const plan = await loadEnvironmentPlan(
        fixture.name,
        paths,
        manifestPath,
      ...
      const instructions = plan.resources.find(
        (resource) => resource.category === "instructions",
      );
      const expectedContent =
        `<!-- Generated by jaeger env. Edit the ${fixture.name} environment, not this file.\n` +
        `     Manifest: ${manifestPath}\n` +
        `     Content: sha256:e3b0c44298fc -->\n\n\n`;

      assert.equal(instructions?.content, expectedContent);
      await mkdir(codexHome, { recursive: true });
      await writeFile(path.join(codexHome, "AGENTS.md"), expectedContent);
      assert.equal(
        (await inspectEnvironmentStatus(plan, paths)).resources.find(
          (resource) => resource.category === "instructions",
        )?.status,
        "current",
      );
...
test("list returns canonical mdcsp profiles", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "jaeger-env-list-"));
  try {
    const profiles = path.join(root, "profiles");
    await mkdir(profiles, { recursive: true });
    await Promise.all([
      writeFile(path.join(profiles, "default.toml"), ""),
      writeFile(path.join(profiles, "work_station.toml"), ""),
      writeFile(path.join(profiles, "Invalid.toml"), ""),
      writeFile(path.join(profiles, "ignored.txt"), ""),
    ]);
